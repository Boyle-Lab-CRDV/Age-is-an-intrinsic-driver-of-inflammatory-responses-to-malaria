...
write.csv(monos_filter,here("output_data/genesbybarplotgroup_forheatmap_280425.csv"))
...
# Run Wilcoxon test
stat.test <- state.long %>%
  wilcox_test(State_FC ~ Age) %>%
  add_xy_position(x = "Age")
 
stat.test

# Violin + boxplot + exact p-value
Up.violin <- ggplot(state.long, aes(x = Age, y = State_FC)) +
  geom_violin(trim = FALSE, aes(fill = Age)) +
  scale_fill_manual(values = c("Child" = "#FF3333", "Adult" = "#3399FF")) +
  geom_boxplot(width = 0.2, aes(fill = Age), color = "black")+
  stat_pvalue_manual(
    stat.test,
    label = "p = {p}")+
...
ggsave(Up.violin, filename="graphs/Violin_UpR_exactpval_090725.pdf")
##Changing to downregulated
#change to dataframes labelled 'Up' or 'Down' to make the violin plots
state <- Down %>%
  select(SYMBOL, qvals.state_bool, qvals.age_bool, qvals.state_bool.age_bool, State_AdultFC, State_ChildFC,  Age_stim, Age_unstim, Sig_cat, Sig_cat2) %>%
  dplyr::rename(Adult = State_AdultFC, Child = State_ChildFC)
...
Down.violin <- ggplot(state.long, aes(x = Age, y = State_FC)) +
  geom_violin(trim = FALSE, aes(fill = Age)) +
  scale_fill_manual(values = c("Child" = "#FF3333", "Adult" = "#3399FF")) +
  geom_boxplot(width = 0.2, aes(fill = Age), color = "black")+
  stat_pvalue_manual(
...
ggsave(Down.violin, filename="graphs/Violin_DownR_exactpval_090725.pdf")
###########################################################################################################################
